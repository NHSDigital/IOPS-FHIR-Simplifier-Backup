...
Making a referral for this application follows the {{pagelink:core-standardpatternforcomposites, text:standard pattern for BaRS operations}}.
...
To cancel a referral this application follows the {{pagelink:core-standardpatternforcomposites, text:standard pattern for BaRS operations}} with an additional step. Before beginning the standard pattern as descbribed on the linked section, the referral **sender** must perform a read of the referral to be cancelled, from the referral **receiver**, prior to cancellation to ensure they are working with the most up-to date information and it has not already been actioned. This is done by performing a "GET ServiceRequest by ID" call to the **receiving** system's corresponding API endpoint (via the BaRS proxy).
