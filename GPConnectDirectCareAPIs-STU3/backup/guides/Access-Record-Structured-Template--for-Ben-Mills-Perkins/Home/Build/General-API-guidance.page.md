...
<b>Summary</b>: Implementation guidance for developers - focusing on general API implementation guidance
</div>
...
<b>Important</b>: Any additional principles highlighted in the GP Connect specification MUST take precedence over those defined in these technical standards.
</div>
...
<div class="nhsd-a-box nhsd-a-box--bg-light-yellow nhsd-!t-margin-bottom-6 nhsd-t-body">
<b>Roadmap</b>: NHS Digital is currently evaluating how a <a href="https://fetch.spec.whatwg.org/"> cross-origin resource sharing</a> (CORS) will be handled for web and mobile based applications.
</div>
...
The [Service Root URL](https://www.hl7.org/fhir/STU3/http.html#general) is the address root address of where the resources defined by a specification are found.

The Service Root URL is the `[base]` portion of FHIR URLs.

It is important to note that the Service Root URL will be different for each GP Connect specification (excluding PFS Appointments and Foundations as mentioned below), as each specification is defined as its own FHIR server:

- PFS Appointment Management (including Foundations, which will always have the same Service Root URL as Appointment Management)
- PFS Access Record Structured
- PFS Prescription Management 
- PFS Access Document
...
<div class="nhsd-a-box nhsd-a-box--bg-light-yellow nhsd-!t-margin-bottom-6 nhsd-t-body">
<b>Important</b>: All URLs (and ids that form part of the URL) defined by this specification are case sensitive.
</div>

### Service Root URL versioning ###

Service Root URLs SHALL be aligned with the GP Connect specification they were built against, specifically the major version number ({{pagelink:Home/Design/Design-approach/Specification-versioning.page.md}}) SHALL be present in the server's Service Root URL to provide a clear distinction between API versions that are incompatible (i.e. contain breaking changes) vs. backwards-compatible (i.e. contain no breaking changes).

Provider systems SHALL publish Service Root URLs for major versions of FHIR APIs in the Spine Directory Service ({{pagelink:Home/Authentication/Spine-Directory-Services/Overview-and-querying.page.md}}) in the following format:
...
- `[PROVIDER_ROUTING_SEGMENT]` enables providers to differentiate between logical FHIR servers defined by GP Connect capabilities, or other FHIR based APIs. For example, the `[PROVIDER_ROUTING_SEGMENT]` could be:
  - `gpconnect` for the PFS Appointment Management and Foundations capabilities
  ...
  **Please note:** The PFS Appointment Management and Foundations capabilities **SHALL** have the same `[PROVIDER_ROUTING_SEGMENT]` value.  Other capabilies **SHALL** have different values.
...
<div class="nhsd-a-box nhsd-a-box--bg-light-blue nhsd-!t-margin-bottom-6 nhsd-t-body">
<b>Tip</b>:  Please see later sections for which HTTP verbs are expected to be available for specific FHIR resources.
</div>

<p/>

<div class="nhsd-a-box nhsd-a-box--bg-light-blue nhsd-!t-margin-bottom-6 nhsd-t-body">
<b>Roadmap</b>: In a future version of the FHIR® standard, it is expected that the PATCH verb will also be supported.
</div>

#### Resource types ####

GP Connect provider systems SHALL support FHIR [resource types](http://hl7.org/fhir/STU3/resourcelist.html) as detailed within the {{pagelink:Home/Build/FHIR-development/FHIR--resources.page.md}}.
...
In line with work being undertaken in other jurisdictions GP Connect provider systems are not expected to resolve full URLs that are external to their environment.
...
<b>Warning</b>: Consumers planning their development should read the {{pagelink:Home/Help-and-support/Frequently-asked-questions.page.md}} on JSON or XML, before choosing whether to use JSON or XML in their implementation.


<div class="nhsd-a-box nhsd-a-box--bg-light-yellow nhsd-!t-margin-bottom-6 nhsd-t-body">
<b>Important</b>: The FHIR standard outlines specific rules for formatting XML and JSON on the wire. It is important to read and understand in full the differences between how XML and JSON are required to be represented.
</div>
...
<div class="nhsd-a-box nhsd-a-box--bg-light-yellow nhsd-!t-margin-bottom-6 nhsd-t-body">
<b>Tip</b>: Applying content compression is key to reducing bandwidth needs and improving battery life for mobile devices.
</div>
...
TO DO - THIS NEEDS TO UPDATED WITH THE NEW USER MANAGEMENT PROCESS
</div>
...
<div class="nhsd-a-box nhsd-a-box--bg-light-yellow nhsd-!t-margin-bottom-6 nhsd-t-body">
<b>Tip</b>: We are using an open standard (JWT) to provide a container for the provenance and audit data for ease of transport between the consumer and provider systems. It is important to note that these tokens (for GP Connect FoT) will <b>not</b> be centrally issued and are not signed or encrypted (that is, they are constructed of plain text). There are JWT libraries available for most programming languages simplifying the generation of this data in JWT format.
</div>

Refer to {{pagelink:Home/Authentication/Integrate-with-Spine/Cross-organisation-audit-and-provenance.page.md}} for full details of the JWT claims that SHALL be used for passing audit and provenance details between systems.

<div class="nhsd-a-box nhsd-a-box--bg-light-yellow nhsd-!t-margin-bottom-6 nhsd-t-body">
<b>Important</b>: We have defined a small number of additional headers which are also required to be included in NHS Digital defined custom headers.
</div>
...
<sup>1</sup> please refer to the {{pagelink:Home/Authentication/Integrate-with-Spine/Interaction-IDs.page.md}} for full details.
